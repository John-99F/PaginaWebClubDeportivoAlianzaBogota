...
export default function Admin() {
  ...
  useEffect(()=>{ getContenido().then(res=>{ setContenido(res); setLoading(false); }); }, []);
  const handleChange=e=>setContenido({...contenido,[e.target.name]:e.target.value});
  const handleSave=async()=>{ await updateContenido(contenido); alert("Contenido actualizado!"); };
      ...
      <main className="admin-main">
        ...
        {["vision","mision","historia","valores"].map(campo=>(
          <div key={campo} className="admin-field">
            <label>{campo}</label>
            <textarea name={campo} value={contenido[campo]} onChange={handleChange} rows={3}/>
          </div>
        ))}
